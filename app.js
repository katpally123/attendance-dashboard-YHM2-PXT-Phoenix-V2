...
(async function boot() {
  ...
  // seed today
  const t = new Date();
  dateEl.value = new Date(Date.UTC(t.getFullYear(), t.getMonth(), t.getDate())).toISOString().slice(0, 10);
  shiftEl.value = "Day";
  updateRibbonStatic();
...
function hoursFromRowFlexible(row) {
  ...
  for (const k of keys) {
    if (row[k] != null && String(row[k]).trim() !== "") return parseHhMm(row[k]);
  }
  return 0;
}
...
async function processAll() {
  ...
  try {
    if (!rosterEl.files[0] || !mytimeEl.files[0]) {
      throw new Error("Upload Roster and MyTime CSVs.");
    }
    ...
    if (!R_ID || !R_DEPT || !(R_SP || R_COR)) {
      throw new Error("Roster must include Employee ID, Department ID, and Shift Pattern/Corner.");
    }
    ...
    const rosterFullRows = rosterRaw.map(r => {
      ...
      const area = String(r[R_AREA ? R_AREA : ""] ?? "").trim();
      const typ = classifyEmpType(r[R_TYPE]);
      const sp  = String(r[R_SP ? R_SP : ""] ?? "");
    ...
    if (vetRaw && vetRaw.length) {
      ...
      for (const r of vetRaw) {
        ...
        if (!empId && A_LOGIN) {
          const eid = loginToEid.get(normLogin(r[A_LOGIN]));
          if (eid) empId = eid;
        }
        if (!empId) continue;
        ...
        const accepted = (Number(r[A_ACC]) > 0) || ["true", "1", "yes"].includes(flag) || ["ACCEPTED", "APPROVED", "COMPLETED"].includes(stat);
        if (!accepted) continue;

        const dISO = toISODate(r[A_S1] || r[A_T1]);
        if (dISO && dISO !== isoDate) continue;

        // ensure shift matches selection if timestamp is available
        const d = new Date(String(r[A_S1] || r[A_T1]).trim());
        if (!isNaN(d)) {
          const hh = d.getHours(), mm = d.getMinutes();
          const mins = hh * 60 + mm;
          const recShift = (mins >= 7 * 60 && mins <= 18 * 60 + 59) ? "Day" : "Night";
          if (recShift !== shiftEl.value) continue;
        }

        if (!fullById.has(empId)) continue;

        const typ = String(r[A_TYP] || "").toUpperCase();
        if (typ.includes("VTO")) vtoSet.add(empId);
        else if (typ.includes("VET") || typ.includes("OVERTIME")) vetSet.add(empId);
      }
    }
    ...
    // ---------- Dashboard table ----------
    const row_RegularExpected   = mkRow();
    cohortExpected.forEach(x => pushCount(row_RegularExpected, x));
    const row_RegularPresentExS = mkRow();
    cohortPresentExSwaps.forEach(x => pushCount(row_RegularPresentExS, x));
    const row_SwapOut           = mkRow();
    swapOutRows.forEach(x => pushCount(row_SwapOut, x));
    const row_SwapInExpected    = mkRow();
    swapInExpectedRows.forEach(x => pushCount(row_SwapInExpected, x));
    const row_SwapInPresent     = mkRow();
    swapInPresentRows.forEach(x => pushCount(row_SwapInPresent, x));
    const row_VTO               = mkRow();
    [...vtoSet].map(id => byId.get(id) || fullById.get(id)).filter(Boolean).forEach(x => pushCount(row_VTO, x));
    const row_VETExpected       = mkRow();
    vetExpectedRows.forEach(x => pushCount(row_VETExpected, x));
    ...
    const tag = (ids, reason) => {
      for (const id of ids) {
        if (byId.has(id) && !reasonOf.has(id)) reasonOf.set(id, reason);
      }
    };
    ...
    for (const x of vetExpectedRows) {
      if (onPrem.get(x.id) !== true && !reasonOf.has(x.id)) {
        reasonOf.set(x.id, "VET accepted but not shown");
      }
    }
    for (const x of cohortExpected) {
      if (x.onp !== true && !reasonOf.has(x.id)) {
        reasonOf.set(x.id, "No-Show (plain)");
      }
    }
    ...
    const auditRows = [];
    for (const [id, reason] of reasonOf.entries()) {
      const x = byId.get(id) || fullById.get(id);
      if (!x) continue;
